...
class Motor6DemoNode(Node):
    """通过 hexmovr_moto_manager 控制 Hexmovr 6 号电机的示例节点。
    ...
    def __init__(self) -> None:
        super().__init__("motor6_demo_node")

        # 参数让这个例程不用改源码也能复用。
        # 本例按需求默认控制 ID 为 6 的电机。
        self.declare_parameter("motor_id", 1)#6
        ...
        self.get_logger().info(
            f"6号电机例程已就绪。motor_id={self.motor_id}, "
            f"command_topic={command_topic}, state_topic={state_topic}, "
            f"demo_enabled={self.demo_enabled}"
        )

        if self.demo_enabled:
            self.get_logger().warn(
                "演示运动已启用。节点会发送速度命令，并在设定时间后释放电机。"
            )
        else:
            self.get_logger().info(
